...
# scMODF 类：多模态融合深度自编码器，融合了 ZINB 和 Bernoulli 两种模型，用于单细胞 RNA-seq 和 ATAC-seq 数据的联合分析。
...
# 解析命令行参数，用于配置训练的 epoch 数量、是否进行 DAE 训练以及预训练模型的路径。
def runAlgorithm():
    ...
    # --pretrain_path1：预训练模型保存的路径，默认值为指定的路径。
    ...
    x1, x2, train_index, test_index, label_ground_truth, _ = read_dataset(File1=X1, File2=X2,
                                                                          ...
                                                                          formar_epi="table")
    # 加载 scRNA-seq 和 scATAC 数据，并读取细胞元数据。
    ...
    # 提取归一化后的数据和原始数据，并将其转换为 Tensor 格式。提取 size factors，并获取数据的形状。
